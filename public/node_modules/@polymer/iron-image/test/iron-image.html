...
<html>
  <head>
    ...
    <script type="module" src="../iron-image.js"></script>
  </head>
  <body>
    <custom-style>
...
suite('<iron-image>', function() {
  ...
  suite('preventLoad', function() {
    ...
    test('The `prevent-load` attribute prevents the image from loading when created.', function() {
    ...
    test('If `preventLoad` is true, then changing `src` does not start loading the image.', function() {
    ...
    test('If the image with URL `src` is not currently loaded and `preventLoad` changes to true, ' +
        'then the image at `src` is loaded.', function(done) {
      var attemptedLoadSpy = sinon.spy();
      image.preventLoad = true;
    ...
    test('If the image with URL `src` is currently loaded, then toggling `preventLoad` does not ' +
        'reload the image.', function(done) {
      var image = fixture('PreventLoad');
