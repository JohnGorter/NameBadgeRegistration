...
<head>
  ...
  <script type="module" src="../iron-selector.js"></script>
...
<body>
  ...
  <script type="module">
import '/node_modules/@polymer/test-fixture/test-fixture.js';
...
suite('defaults', function() {
  ...
  setup(function(done) {
    ...
    s1.addEventListener('iron-items-changed', function onIronItemsChanged() {
      s1.removeEventListener('iron-items-changed', onIronItemsChanged);
      done();
    });
  });
...
suite('basic', function() {
  ...
  setup(function(done) {
    ...
    s2.addEventListener('iron-items-changed', function onIronItemsChanged() {
      s2.removeEventListener('iron-items-changed', onIronItemsChanged);
      done();
    });
  });
  ...
  test('force synchronous item update', function() {
    ...
    dom(s2).appendChild(document.createElement('div'));
    expect(s2.items.length).to.be.equal(5);
  ...
  suite('items changing', function() {
    ...
    test('cause iron-items-changed to fire', function(done) {
      ...
      dom(s2).appendChild(newItem);

      Base.async(function() {
        dom(s2).removeChild(newItem);
    ...
    test('updates selected item', function(done) {
      ...
      var firstElementChild = dom(s1).firstElementChild;
      ...
      s1.addEventListener('iron-items-changed', function() {
        firstElementChild = dom(s1).firstElementChild;
        expect(firstElementChild).to.be.equal(s1.selectedItem);
        expect(firstElementChild.classList.contains('iron-selected'))
            .to.be.eql(true);
        done();
      ...
      dom(s1).removeChild(s1.selectedItem);
    });
  });
  ...
  suite('dynamic selector', function() {
    ...
    test('selects dynamically added child automatically', function(done) {
      ...
      dom(selector).appendChild(child);
