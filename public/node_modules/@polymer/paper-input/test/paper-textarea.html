...
<head>
  ...
  <script src="../../iron-test-helpers/test-helpers.js" type="module"></script>
...
suite('basic', function() {
  ...
  test('caret position is preserved', function() {
    var input = fixture('basic');
    var ironTextarea = dom(input.root).querySelector('iron-autogrow-textarea');
...
suite('a11y', function() {
  ...
  test('has aria-labelledby', function() {
    ...
    assert.equal(input.inputElement.textarea.getAttribute('aria-labelledby'), dom(input.root).querySelector('label').id, 'aria-labelledby points to the label');
  });
  ...
  test('has aria-describedby for error message', function(done) {
    ...
    Base.async(function() {
      ...
      assert.equal(input.inputElement.textarea.getAttribute('aria-describedby'), dom(input.root).querySelector('paper-input-error').id, 'aria-describedby points to the error message');
      done();
    }, 1);
  });
  ...
  test('has aria-describedby for character counter', function(done) {
    ...
    Base.async(function() {
      assert.isTrue(input.inputElement.textarea.hasAttribute('aria-describedby'));
      assert.equal(input.inputElement.textarea.getAttribute('aria-describedby'), dom(input.root).querySelector('paper-input-char-counter').id, 'aria-describedby points to the character counter');
      done();
    }, 1);
  });
  ...
  test('has aria-describedby for character counter and error', function(done) {
    ...
    Base.async(function() {
      ...
      assert.notEqual(ariaDescribedBy.indexOf(dom(input.root).querySelector('paper-input-error').id, -1,
        'aria-describedby points to the error message'));
      assert.notEqual(ariaDescribedBy.indexOf(dom(input.root).querySelector('paper-input-char-counter').id, -1,
