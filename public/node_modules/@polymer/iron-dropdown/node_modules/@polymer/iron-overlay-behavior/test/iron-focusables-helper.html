...
<head>
  ...
  <script type="module" src="../iron-focusables-helper.js"></script>
  <script type="module" src="./test-buttons.js"></script>
  <script type="module" src="./test-buttons-wrapper.js"></script>
...
suite('getTabbableNodes', function() {
  ...
  test('returns tabbable nodes', function() {
    ...
    var focusableNodes = IronFocusablesHelper.getTabbableNodes(node);
    assert.equal(focusableNodes.length, 3, '3 nodes are focusable');
    assert.equal(focusableNodes[0], dom(node).querySelector('.focusable1'));
    assert.equal(focusableNodes[1], dom(node).querySelector('.focusable2'));
    assert.equal(focusableNodes[2], dom(node).querySelector('.focusable3'));
  });
  ...
  test('excludes visibility: hidden elements', function() {
    var node = fixture('basic');
    var focusable = dom(node).querySelector('.focusable1');
  ...
  test('respects the tabindex order', function() {
    ...
    for (var i = 0; i < 12; i++) {
      assert.equal(focusableNodes[i], dom(node).querySelector('.focusable' + (i + 1)));
    }
  });
  ...
  test('includes tabbable elements in the shadow dom', function() {
    ...
    assert.equal(focusableNodes[2], dom(node).querySelector('input'));
  ...
  test('handles composition', function() {
    ...
    assert.equal(focusableNodes[3], dom(node).querySelector('input'));
  ...
  test('handles distributed nodes', function() {
    ...
    var focusableNodes = IronFocusablesHelper.getTabbableNodes(wrapped);
    assert.equal(focusableNodes.length, 4, '4 nodes are focusable');
