...
<head>
  ...
  <script type="module" src="../../iron-test-helpers/iron-test-helpers.js"></script>
  <script type="module" src="./test-overlay.js"></script>
  <script type="module" src="./test-overlay2.js"></script>
  <script type="module" src="./test-buttons.js"></script>
  <script type="module" src="./test-menu-button.js"></script>
...
import { dom, flush } from '/node_modules/@polymer/polymer/lib/legacy/polymer.dom.js';
...
suite('basic overlay', function() {
  ...
  test('open overlay refits on iron-resize', function(done) {
    runAfterOpen(overlay, function() {
      ...
      afterNextRender(overlay, function() {
  ...
  test('closed overlay does not trigger iron-resize when its content changes', function(done) {
    ...
    dom(overlay).appendChild(document.createElement('div'));
...
suite('opened overlay', function() {
  ...
  test('overlay positioned & sized properly', function(done) {
    overlay.addEventListener('iron-overlay-opened', function() {
      ...
      assert.closeTo(parseFloat(s.left), (window.innerWidth - overlay.offsetWidth) / 2, 1,
        'centered horizontally');
      assert.closeTo(parseFloat(s.top), (window.innerHeight - overlay.offsetHeight) / 2, 1,
        'centered vertically');
      done();
    });
  });
...
suite('focus handling', function() {
  ...
  test('node with autofocus is focused', function(done) {
    runAfterOpen(overlay, function() {
      assert.equal(dom(overlay).querySelector('[autofocus]'), document.activeElement,
  ...
  test('no-auto-focus will not focus node with autofocus', function(done) {
    ...
    runAfterOpen(overlay, function() {
      assert.notEqual(dom(overlay).querySelector('[autofocus]'), document.activeElement,
...
suite('focusable nodes', function() {
  ...
  test('_focusableNodes returns nodes that are focusable', function(done) {
    runAfterOpen(overlay, function() {
      ...
      assert.equal(focusableNodes[0], dom(overlay).querySelector('.focusable1'));
      assert.equal(focusableNodes[1], dom(overlay).querySelector('.focusable2'));
      assert.equal(focusableNodes[2], dom(overlay).querySelector('.focusable3'));
      done();
    });
  });
  ...
  test('_focusableNodes respects the tabindex order', function(done) {
    runAfterOpen(overlayWithTabIndex, function() {
      ...
      assert.equal(focusableNodes[0], dom(overlayWithTabIndex).querySelector('.focusable1'));
      assert.equal(focusableNodes[1], dom(overlayWithTabIndex).querySelector('.focusable2'));
      assert.equal(focusableNodes[2], dom(overlayWithTabIndex).querySelector('.focusable3'));
      assert.equal(focusableNodes[3], dom(overlayWithTabIndex).querySelector('.focusable4'));
      assert.equal(focusableNodes[4], dom(overlayWithTabIndex).querySelector('.focusable5'));
      assert.equal(focusableNodes[5], dom(overlayWithTabIndex).querySelector('.focusable6'));
      done();
    });
  });
  ...
  test('with-backdrop: TAB & Shift+TAB wrap focus', function(done) {
    ...
    runAfterOpen(overlay, function() {
      ...
      Base.async(function() {
        ...
        assert.equal(focusableNodes[focusableNodes.length - 1], document.activeElement,
  ...
  test('with-backdrop: after open, update last focusable node and then Shift+TAB', function(done) {
    ...
    runAfterOpen(overlay, function() {
      ...
      Base.async(function() {
        ...
        // Simulate Shift+TAB.
        MockInteractions.pressAndReleaseKeyOn(document, 9, ['shift']);
        assert.equal(focusableNodes[focusableNodes.length - 2], document.activeElement,
  ...
  test('with-backdrop: Shift+TAB wrap focus in shadowDOM', function(done) {
    ...
    runAfterOpen(overlayFocusableNodes, function() {
      ...
      Base.async(function() {
        ...
        // Simulate Shift+TAB.
        MockInteractions.pressAndReleaseKeyOn(document, 9, ['shift']);
        assert.equal(overlayFocusableNodes.$.last, IronOverlayManager.deepActiveElement,
...
suite('Polymer.IronOverlayManager.deepActiveElement', function() {
  ...
  test('handles document.body', function() {
    document.body.focus();
    assert.equal(IronOverlayManager.deepActiveElement, document.body);
  });

  test('handles light dom', function() {
    var focusable = document.getElementById('focusInput');
    focusable.focus();
    assert.equal(IronOverlayManager.deepActiveElement, focusable, 'input is handled');
    focusable.blur();
  });
  ...
  test('handles shadow dom', function() {
    ...
    focusable.focus();
    assert.equal(IronOverlayManager.deepActiveElement, focusable);
    focusable.blur();
  });
...
suite('restore-focus-on-close', function() {
  ...
  test('does not return focus on close by default (restore-focus-on-close=false)', function(done) {
    ...
    runAfterOpen(overlay, function() {
      runAfterClose(overlay, function() {
        assert.notEqual(IronOverlayManager.deepActiveElement, focusable,
  ...
  test('avoids restoring focus if focus changed', function(done) {
    ...
    runAfterOpen(overlay, function() {
      button1.focus();
      runAfterClose(overlay, function() {
        assert.equal(IronOverlayManager.deepActiveElement, button1,
...
suite('overlay with backdrop', function() {
  ...
  test('manager.getBackdrops() updated on opened changes', function(done) {
    runAfterOpen(overlay, function() {
      assert.equal(IronOverlayManager.getBackdrops().length, 1,
        'overlay added to manager backdrops');
      runAfterClose(overlay, function() {
        assert.equal(IronOverlayManager.getBackdrops().length, 0,
...
suite('Manager overlays in sync', function() {
  ...
  setup(function() {
    ...
    overlay1 = f[0];
    overlay2 = f[1];
    overlays = IronOverlayManager._overlays;
  });
  ...
  test('open/close brings overlay on top', function(done) {
    ...
    runAfterOpen(overlay2, function() {
      ...
      runAfterOpen(overlay1, function() {
        ...
        assert.isAbove(parseInt(overlay1.style.zIndex), parseInt(overlay2.style.zIndex),
...
suite('z-ordering', function() {
  ...
  setup(function() {
    ...
    overlay1 = f[0];
    overlay2 = f[1];
    originalMinimumZ = IronOverlayManager._minimumZ;
  });

  teardown(function() {
    IronOverlayManager._minimumZ = originalMinimumZ;
  });
...
suite('multiple overlays with backdrop', function() {
  ...
  test('multiple overlays share the same backdrop', function() {
    assert.isTrue(overlay1.backdropElement === overlay2.backdropElement,
      'overlay1 and overlay2 have the same backdrop element');
    assert.isTrue(overlay1.backdropElement === overlay3.backdropElement,
  ...
  test('iron-overlay-backdrop is removed from the DOM when all overlays with backdrop are closed', function(
    done) {
    // Open & close them all.
    overlay1.opened = true;
    overlay2.opened = true;
  ...
  test('click event handled only by top overlay', function(done) {
    runAfterOpen(overlay1, function() {
      runAfterOpen(overlay2, function() {
        var btn = dom(overlay2).querySelector('button');
        btn.addEventListener('click', overlay2.close.bind(overlay2));
        MockInteractions.tap(btn);
  ...
  test('tap event handled only by top overlay', function(done) {
    runAfterOpen(overlay1, function() {
      runAfterOpen(overlay2, function() {
        var btn = dom(overlay2).querySelector('button');
...
suite('overlay in composed tree', function() {
  ...
  test('click on overlay content does not close it', function(done) {
    ...
    MockInteractions.tap(dom(overlay).querySelector('button'));
    Base.async(function() {
  ...
  test('with-backdrop wraps the focus within the overlay', function(done) {
    // 1ms timeout needed by IE10 to have proper focus switching.
    Base.async(function() {
      var buttons = dom(overlay).querySelectorAll('button');
      // Go to last element.
      ...
      // Simulate TAB.
      MockInteractions.pressAndReleaseKeyOn(document, 9);
      assert.equal(buttons[0], IronOverlayManager.deepActiveElement,
        'focus wrapped to first focusable');
      assert.isTrue(tabSpy.calledOnce, 'keydown spy called');
      assert.isTrue(tabSpy.getCall(0).args[0].defaultPrevented, 'keydown default prevented');
      // Simulate Shift+TAB.
      MockInteractions.pressAndReleaseKeyOn(document, 9, ['shift']);
      assert.equal(buttons[buttons.length - 1], IronOverlayManager.deepActiveElement,
        'focus wrapped to last focusable');
      assert.isTrue(tabSpy.calledTwice, 'keydown spy called again');
      assert.isTrue(tabSpy.getCall(1).args[0].defaultPrevented, 'keydown default prevented again');
      // Cleanup.
  ...
  test('stays on top', function(done) {
    runAfterOpen(overlay1, function() {
      runAfterOpen(overlay2, function() {
        ...
        assert.equal(IronOverlayManager.currentOverlay(), overlay1, 'currentOverlay ok');
        done();
      });
    });
  });
  ...
  test('last overlay with always-on-top wins', function(done) {
    ...
    runAfterOpen(overlay1, function() {
      runAfterOpen(overlay2, function() {
        ...
        assert.equal(IronOverlayManager.currentOverlay(), overlay2, 'currentOverlay ok');
        done();
      });
    });
  });
