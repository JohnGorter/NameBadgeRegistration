...
suite('<iron-dropdown>', function() {
  ...
  suite('basic', function() {
    setup(function() {
      ...
      content = dom(dropdown).querySelector('[slot="dropdown-content"]');
    });
    ...
    suite('when content is focusable', function() {
      ...
      test('focuses a configured focus target', function(done) {
        var focusableChild = dom(content).querySelector('div[tabindex]');
    ...
    suite('correct animationConfig setup', function() {
      ...
      test('no pointer events during animation', function(done) {
        dropdown.openAnimationConfig = [{
        ...
        dropdown.open();
        setTimeout(function() {
          assert.equal(getComputedStyle(content).pointerEvents, 'none', 'content is interactive');
        }, 150);
        dropdown.addEventListener('iron-overlay-opened', function() {
          assert.notEqual(getComputedStyle(content).pointerEvents, 'none', 'content is not interactive');
          done();
        });
      });
    });
  ...
  suite('locking scroll', function() {
    ...
    test('should lock, only once', function(done) {
      var openCount = 0;
      runAfterOpen(dropdown, function() {
        expect(IronDropdownScrollManager._lockingElements.length)
          .to.be.equal(1);
        expect(IronDropdownScrollManager.elementIsScrollLocked(document.body))
          .to.be.equal(true);
  ...
  suite('sizing target', function() {
    ...
    test('sizingTarget can be set to a child element', function(done) {
      var subcontent = dom(dropdown).querySelector('.subcontent');
